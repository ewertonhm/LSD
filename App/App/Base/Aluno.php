...
abstract class Aluno implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\App\\Map\\AlunoTableMap';
    ...
    /**
     * Initializes internal state of App\App\Base\Aluno object.
     */
     ...
     * @return $this|\App\App\Aluno The current object (for fluent API support)
     */
    public function setCodaluno($v)
    {
        ...
        if ($this->codaluno !== $v) {
            $this->codaluno = $v;
            $this->modifiedColumns[AlunoTableMap::COL_CODALUNO] = true;
        }
    ...
    /**
     * Set the value of [ra] column.
     *
     * @param int $v new value
     * @return $this|\App\App\Aluno The current object (for fluent API support)
     */
    public function setRa($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
        ...
        if ($this->ra !== $v) {
            $this->ra = $v;
            $this->modifiedColumns[AlunoTableMap::COL_RA] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AlunoTableMap::translateFieldName('Codaluno', TableMap::TYPE_PHPNAME, $indexType)];
            $this->codaluno = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AlunoTableMap::translateFieldName('Ra', TableMap::TYPE_PHPNAME, $indexType)];
            $this->ra = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AlunoTableMap::translateFieldName('Nome', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = AlunoTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\App\\App\\Aluno'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->trabalhosScheduledForDeletion !== null) {
                if (!$this->trabalhosScheduledForDeletion->isEmpty()) {
                    \App\App\TrabalhoQuery::create()
                        ->filterByPrimaryKeys($this->trabalhosScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[AlunoTableMap::COL_CODALUNO] = true;
        if (null !== $this->codaluno) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . AlunoTableMap::COL_CODALUNO . ')');
        }
        if (null === $this->codaluno) {
            try {
                $dataFetcher = $con->query("SELECT nextval('aluno_codaluno_seq')");
                $this->codaluno = (int) $dataFetcher->fetchColumn();
            } catch (Exception $e) {
        ...
        if ($this->isColumnModified(AlunoTableMap::COL_CODALUNO)) {
            $modifiedColumns[':p' . $index++]  = 'codaluno';
        }
        if ($this->isColumnModified(AlunoTableMap::COL_RA)) {
            $modifiedColumns[':p' . $index++]  = 'ra';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'codaluno':
                        ...
                        break;
                    case 'ra':
                        $stmt->bindValue($identifier, $this->ra, PDO::PARAM_INT);
                        break;
                    case 'nome':
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setCodaluno($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setRa($arr[$keys[1]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\App\Aluno The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AlunoTableMap::COL_CODALUNO)) {
            $criteria->add(AlunoTableMap::COL_CODALUNO, $this->codaluno);
        }
        if ($this->isColumnModified(AlunoTableMap::COL_RA)) {
            $criteria->add(AlunoTableMap::COL_RA, $this->ra);
        }
        if ($this->isColumnModified(AlunoTableMap::COL_NOME)) {
    ...
    /**
     * Generic method to set the primary key (codaluno column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\App\Aluno (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setCodaluno(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\App\Aluno Clone of current object.
     * @throws PropelException
     */
    ...
    public function initTrabalhos($overrideExisting = true)
    {
        ...
        $this->collTrabalhos->setModel('\App\App\Trabalho');
    }
    ...
    public function removeTrabalho(ChildTrabalho $trabalho)
    {
        if ($this->getTrabalhos()->contains($trabalho)) {
            ...
            $this->trabalhosScheduledForDeletion[]= clone $trabalho;
            $trabalho->setAluno(null);
        }
